...
class I3D_ResNet_SimCLR(nn.Module):
    def __init__(self, depth, num_classes=1000, dropout=0.5, without_t_stride=False,
                 zero_init_residual=False,simclr_dim=128):
        super(I3D_ResNet_SimCLR, self).__init__()
        layers = {
            18: [2, 2, 2, 2],
            34: [3, 4, 6, 3],
            50: [3, 4, 6, 3],
        ...
        self.simclr_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(512 * block.expansion, simclr_dim),
        )
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2)
        n, c, nf = x.size()
        simclr_feature = self.simclr_layer(torch.mean(x,1))
        x = x.contiguous().view(n * c, -1)
        x = self.dropout(x)
        x = self.fc(x)
        x = x.view(n, c, -1)
        ...
        return logits,simclr_feature


def i3d_resnet_simclr(depth, num_classes, dropout, without_t_stride, **kwargs):
    model = I3D_ResNet_SimCLR(depth, num_classes=num_classes, dropout=dropout, without_t_stride=without_t_stride)
