...
def main_worker(gpu, ngpus_per_node, args):
    ...
    video_path=Path("{}/kinetics/hdf5".format(args.datadir))
    annotation_path=Path("./data/kinetics/mini-kinetics-200_train.json")
    video_path_formatter = (lambda root_path, label, video_id: root_path /
                                label / '{}.hdf5'.format(video_id))

    train_dataset= VideoDataset(video_path,
                                 annotation_path,
                                 'training',
                                 spatial_transform=train_augmentor,
                                 temporal_transform=temporal_transform,
                                 target_transform=None,
                                 video_loader=VideoLoaderHDF5(),
                                 video_path_formatter=video_path_formatter)
