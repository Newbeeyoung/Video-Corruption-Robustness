...
def main():
    ...
    if args.corruption!=-1:
    # Adding corruptions
        corruptions_aug=[]
        CORRUPTIONS = [gaussian_noise, impulse_noise, gaussian_blur, defocus_blur, contrast,
                       brightness]

        corruptions_aug.append(transforms.Lambda(lambda x:np.uint8(CORRUPTIONS[args.corruption](x,severity=args.severity))))
        corruptions_aug.append(transforms.ToPILImage())


        corruption_transform=transforms.Compose(corruptions_aug)
    ...
    val_video_path = Path("{}/kinetics/mini_kinetics/val_hdf5".format(args.datadir))
    val_annotation_path = Path("./data/kinetics/mini-kinetics-200_val.json")
    ...
    if args.evaluate:
        print('Val@{}({}) (# corruption ={}, #severity ={}, # crops = {}, # clips = {}): \tTop@1: {:.4f}\tTop@5: {:.4f}'.format(
            args.input_size, scale_size,args.corruption,args.severity, args.num_crops, args.num_clips, top1.avg, top5.avg),
            flush=True)
        print('Val@{}({}) (# corruption ={}, #severity ={}, # crops = {}, # clips = {}): \tTop@1: {:.4f}\tTop@5: {:.4f}'.format(
            args.input_size, scale_size,args.corruption,args.severity, args.num_crops, args.num_clips, top1.avg, top5.avg),
